...
Run this locally whenever you add new tracks to @official_stardrift.
...
REPO = "kevinswan102/music-shorts"
RELEASE_TAG = "audio-tracks"
DOWNLOAD_DIR = "/tmp/stardrift_tracks"


def list_channel_videos():
    """Get all video IDs and titles from the channel."""
    cmd = ["yt-dlp", "--flat-playlist", "--dump-json", "--no-warnings", f"{CHANNEL_URL}/videos"]
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
    if result.returncode != 0:
        logger.error(f"Channel scan failed: {result.stderr[:300]}")
        return []
